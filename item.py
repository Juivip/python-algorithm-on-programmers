...
# 이 게임의 아이템은 캐릭터의 공격력은 높이고 체력을 낮춥니다. 그래서 아이템을 적절히 사용해 팀의 공격력을 최대한 끌어올리려 합니다.
...
def solution(healths, items):
    used_items = []
    items = list(zip(range(1, len(items)+1), items))

    items.sort(key=lambda item: item[1][0] / item[1][1], reverse=True)
    # print(items)
    healths = list(map(lambda x: x-100, sorted(healths)))
    # print(healths)

    for id, [ad, hp] in items:
        healths = filter(lambda x: x > 0, healths)
...
class MyTest(unittest.TestCase):
    def test(self):
        self.assertEqual(solution([200, 120, 150],
                                  [[30, 100], [500, 30], [100, 400]]), [1, 2])

        self.assertEqual(solution([300, 200, 500],
                                  [[1000, 600], [400, 500], [300, 100]]), [3])
